...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Required. Name of gene to be analyzed. Must match exactly the name of a gene in SCOREFILE (in first column).",
)
parser.add_argument(
    "--Eval",
    default=0.001,
...
parser.add_argument(
    ...
    default=400,
    type=float,
    help="Optional. Length (aa) of the gene to be analyzed. Used to accurately calculate E-value threshold. Default is 400aa. Only large deviations will qualitatively affect results.",
)
parser.add_argument(
    "--dblenfile",
    type=str,
...
for i in range(0, len(speciesorder)):
    ...
    if found == False:
        sys.exit(
            "One or more species names in distance file do not match species names in distance file! The first I encountered was"
            + speciesorder[i]
            + ". Quitting. \n"
        )
...
if len(predbitscores) < 3:
    sys.exit(
        "Too few species with detected homologs! There must be at least 3. Quitting. \n"
    )
...
if parout != "failed":
    ...
    print("\n")
    print(
        "Best-fit a parameter from bitscores included in fit (black points):",
        str(round(a, 2)),
    )
    print(
        "Best-fit b parameter from bitscores included in fit (black points): ",
        round(b, 2),
    )
    print(
        "r squared from all bitscores (black points and orange points, if any):",
        round(r_value**2, 2),
    )
    print("\n")
    ...
    for j in range(0, len(tocalcdists)):
        ...
        lowprediction, highprediction, pval = PI_find(
            testavals, testbvals, tocalcdists[j]
        )
        print(tocalcspecs[j], ":")
        print("Probability of homolog being undetected: ", round(pval, 2))
        print(
            "Maximum likelihood bitscore prediction: ",
            str(round(func(tocalcdists[j], a, b), 2)),
        )
        print(
...
plt.fill_between(
    smoothx, highs, lows, facecolor="blue", alpha=0.2, label="99% confidence interval"
)
plt.ylabel("Bitscore", fontsize=13, labelpad=10, **afont)
plt.xlabel(
    "Evolutionary distance from focal species", fontsize=13, labelpad=10, **afont
)
...
plt.axhline(
    y=globalbitthresh, linestyle="dashed", c="black", label="Detectability threshold"
)
