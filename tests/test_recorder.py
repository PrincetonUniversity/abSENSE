...
def test_plot(default_params, mocker):
    ...
    covariance=[[1.69e+03, 1.20e+00],
                [1.20e+00, 1.82e-03]]

    result = StringIO()
    recorder.plot('GENE', df, 2359.9123, 0.6761,
                  correlation=0.9, bit_threshold=40, outfile=result, covariance=covariance)
...
def test_plot_interpolated(default_params, mocker):
    ...
    df = pd.read_csv(result, index_col=0)
    covariance = [[26.43, 1.02],
                  [ 1.02, 0.16]]

    result = StringIO()
    recorder.plot('NP_116682.3', df, 352.347, 14.4606,
                  correlation=-0.99, bit_threshold=41.54, outfile=result,
